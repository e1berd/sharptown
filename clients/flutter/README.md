# sharptown (Dart / Flutter client)

Expressive Dart and Flutter client for the [Sharptown](https://github.com/e1berd/sharptown)
image transformation API. One fluent API across two transports — **REST** (default) and
**JSON-RPC** over WebSocket — chosen when you create the client.

```dart
final st = SharptownClient('http://localhost:3001');

final bytes = await st
    .transform(ImageInput.file(file))
    .resize(800, 600)
    .blur(3)
    .grayscale()
    .convert('webp')
    .bytes();
```

## Install

Not published to pub.dev — it lives in the repository under `clients/flutter`. Add it as a
Git dependency pointing at that subdirectory:

```yaml
dependencies:
  sharptown:
    git:
      url: https://github.com/e1berd/sharptown
      path: clients/flutter
```

```dart
import 'package:sharptown/sharptown.dart';
```

Works in Flutter (mobile, desktop) and plain Dart. It depends only on `http` (REST) and
`web_socket_channel` (JSON-RPC). `ImageInput.file`, `ImageInput.path` and
`TransformResponse.save` use `dart:io`, so on Flutter Web use `ImageInput.bytes` /
`ImageInput.url` and read `response.bytes` directly.

## Create a client

```dart
final st = SharptownClient(
  'http://localhost:3001',
  timeout: const Duration(seconds: 15),
  headers: {'authorization': 'Bearer …'},
);
```

Parameters: `transport`, `headers`, `timeout`, `httpClient`. The base URL must match the
chosen transport. The scheme is optional — a bare host like `localhost:3001` defaults to the
secure variant (`https://`, or `wss://` for JSON-RPC); pass `http://` (or `ws://`) explicitly
for a plain connection. Call `st.close()` when you're done, unless you passed your own
`httpClient` (then you own its lifecycle).

## Choosing a transport

```dart
// REST (default) — multipart POST to /api/v1/transform
SharptownClient('http://localhost:3001');

// JSON-RPC over WebSocket — image.transform at /rpc
SharptownClient('ws://localhost:3002', transport: const JsonRpcTransport());
```

Both transports accept the same builder, validate operations identically, and return the
same `TransformResponse`, so swapping one for another never changes your calling code.

## Inputs

```dart
ImageInput.bytes(buffer, 'photo.jpg');          // an in-memory Uint8List
ImageInput.file(File('photo.jpg'));             // a dart:io File
ImageInput.path('photo.jpg');                   // convenience: read from disk
ImageInput.url('https://example.com/cat.jpg');  // convenience: fetch over HTTP
```

Sources are resolved lazily, when the transform runs.

A typical Flutter handler, fully in memory:

```dart
Future<Uint8List> thumbnail(Uint8List picked) {
  return st
      .transform(ImageInput.bytes(picked, 'upload.jpg'))
      .width(1024)
      .convert('webp')
      .bytes();
}
```

## Operations

Resize & crop: `resize`, `width`, `height`, `crop`, `smartCrop`, `fit`, `background`,
`dpr`, `aspectRatio`, `autoOrient`, `rotate`, `flip`.
Tone & colour: `brightness`, `contrast`, `saturation`, `exposure`, `hue`, `gamma`,
`colorize`, `tint`, `grayscale`.
Filters & effects: `blur`, `sharpen`, `sepia`, `invert`, `threshold`, `oilPaint`.
Alpha & output: `removeAlpha`, `ensureAlpha`, `convert`, `quality`, `progressive`,
`stripMetadata`, `keepMetadata`.

Validation happens as you build; the first invalid value (out of range, unsupported
format) throws a `SharptownError` immediately, before any request is sent.

## Terminals

```dart
final res = await t.response();   // TransformResponse (status, headers, bytes)
final data = await t.bytes();     // Uint8List
final file = await t.save('out.webp');
```

## Errors

Server and validation failures come back as `SharptownError`:

```dart
try {
  await st.transform(input).convert('webp').bytes();
} on SharptownError catch (e) {
  print('${e.status}: ${e.message}'); // HTTP status / RPC code + message
}
```

## Signed image proxy

Build a signed URL for the server's `GET /fetch` endpoint, suitable for an `<img src>`: the
server downloads, transforms, and caches the remote image. Pass `proxySecret` (the server's
`SHARPTOWN_PROXY_KEY`); sign on a trusted server only.

```dart
final st = SharptownClient('https://img.example.com', proxySecret: secret);
final src = st.signedUrl('https://example.com/photo.jpg', {'width': 800, 'convertTo': 'webp'});
```

## License

MIT
