...
Схему можно не указывать — голый хост вида `localhost:3001` по умолчанию использует
защищённый `https://`; для незащищённого подключения (например, локального dev-сервера)
укажите `http://` явно.
