...
соответствовать выбранному транспорту. Схему можно не указывать — голый хост вида
`localhost:3001` по умолчанию использует защищённый вариант (`https://`, либо `wss://` для
JSON-RPC); для незащищённого подключения укажите `http://` (или `ws://`) явно. Вызовите
`st.close()` по завершении, если не передавали собственный `httpClient`.
